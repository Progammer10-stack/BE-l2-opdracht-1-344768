...
return new class extends Migration
{
    ...
    public function down(): void
    {
        ...
        DB::table('ProductPerLeverancier')->delete();
        DB::table('ProductPerAllergeen')->delete();
        DB::table('Magazijn')->delete();
        DB::table('Leverancier')->delete();
        DB::table('Allergeen')->delete();
    ...
    {
        $this->addForeignKeyIfMissing('Magazijn', 'ProductId', 'FK_Magazijn_Product', 'Product');
        $this->addForeignKeyIfMissing('ProductPerAllergeen', 'ProductId', 'FK_ProductPerAllergeen_Product', 'Product');
        $this->addForeignKeyIfMissing('ProductPerAllergeen', 'AllergeenId', 'FK_ProductPerAllergeen_Allergeen', 'Allergeen');
        $this->addForeignKeyIfMissing('ProductPerLeverancier', 'LeverancierId', 'FK_ProductPerLeverancier_Leverancier', 'Leverancier');
        $this->addForeignKeyIfMissing('ProductPerLeverancier', 'ProductId', 'FK_ProductPerLeverancier_Product', 'Product');
    }

    private function addForeignKeyIfMissing(string $table, string $column, string $name, string $parentTable): void
    {
        if (Schema::hasForeignKey($table, $name) || Schema::hasForeignKey($table, [$column])) {
            return;
        }

        Schema::table($table, function (Blueprint $blueprint) use ($column, $name, $parentTable): void {
    ...
    private function replaceSpecificationData(): void
    {
        Schema::disableForeignKeyConstraints();
